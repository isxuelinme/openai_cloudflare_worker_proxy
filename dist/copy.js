...
(() => {
  ...
  var convertToArray = (value) => Array.isArray(value) ? value : [value];
  ...
  var useUpstream = async (context, next) => {
    ...
    const onRequest = upstream.onRequest ? convertToArray(upstream.onRequest) : null;
    const onResponse = upstream.onResponse ? convertToArray(upstream.onResponse) : null;
    let upstreamRequest = cloneRequest(url, request);
    if (onRequest) {
      upstreamRequest = onRequest.reduce((prevRequest, fn) => fn(cloneRequest(url, prevRequest), url), upstreamRequest);
    }
    context.response = await fetch(upstreamRequest);
    if (onResponse) {
      context.response = onResponse.reduce((prevResponse, fn) => fn(new Response(prevResponse.body, prevResponse), url), new Response(context.response.body, context.response));
    }
    await next();
  };
